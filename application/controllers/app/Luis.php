...
class Luis extends CI_Controller{
    ...
    public function __construct(){
		...
		verifica_token();
        $this->load ->model('app/Luis_model');//
    ...
    public function index(){
        ...
        $data['_APP_TITLE']              = "Luis";        
        $data['_APP_VIEW_NAME']          = "Administracion";
        $data['_APP_MENU']               = get_role_menu($this->rol_id, 3, 65);// menu lateral
        ...
        $data['_APP_BREADCRUMBS']        = array("Luis");
		
        $data['scripts'][] = 'propiosScripts/Luis';
        ...
        $data['modals'][]  = $this->load->view('private/fragments/Luis/modalLuis', $data, TRUE);
        $data['_APP_FRAGMENT'] = $this->load->view('private/fragments/Luis/Luis_view', $data, TRUE);

        $this->load->view("default",$data,FALSE);

    }

    public function verBLuis(){

           $rs = $this->Luis_model->ver_bluis();
           $data['resultado'] = $rs != null;
           $data['mensaje'] = $data['resultado'] ? "Se econtraron ". count($rs)." becarios" : "no se econtraron becarios";
           $data["Becarios"] = $rs;
    ...
    public function insertaBLuis(){
        ...
        if($data['accion']== "Agregar"){
            ...
            unset($data['accion']); 
            $rs = $this->Luis_model->inserta_bluis($data);

        }else if($data['accion']== "editar"){

            unset($data['accion']); 
            $rs = $this->Luis_model->update_bluis($data, $data['idBe']);
        }
    ...
    public function cambioEstatus(){
        ...
        $rs = $this->Luis_model->estatus_bluis($id);
    ...
    public function bajaLogica(){
        ...
        $id = $this->input->post("id");
        $rs = $this->Luis_model->borradoLogico($id);
