...
function listaSecciones() {
  axios(base_url() + "daniw/Secciones/verSecciones")
    .then(({ data }) => {
      if (data.resultado) {
        ...
        $.each(data.Secciones, function (i, o) {
          $("#datatable").find("tbody").append(
              `<tr id="tr-`+ o.seccion_id +`">
                <td>` + o.seccion_id  + `</td>
                <td>` + o.nombre_sec  + `</td>
                <td>` + o.des_seccion + `</td>
                <td>` + o.nombre_mod  + `</td>
						    <td> <i class="` + o.ico_sec     + `"></i></td>
                <td>` + o.url_sec     + `</td>
                <td>` + o.orden     + `</td>
                <td align="center">
                  <a href="#" onclick="cambiaEstatus(` + o.seccion_id + `)">
                    <i id="icono-` + o.seccion_id + `" class="fas fa-toggle-` + (o.estatus == 1 ? "on" : "off") + ` fa-2x"></i>
                  </a>
                </td>
                <td align="center">
                  <a href="#" onclick="editar('` + o.seccion_id + `','` + o.nombre_sec + `','` + o.des_seccion + `','` + o.ico_sec + `','` + o.url_sec + `','` + o.orden + `','` + o.nombre_mod + `')">
                    <i class="fas fa-pencil fa-2x"></i>
                  </a> 
                </td>
                <td align="center">
                  <a href="#" onclick="modalBorrar(` + o.seccion_id + `,'` + o.nombre_sec + `','` + o.des_seccion + `')">
...
function insertaSecciones() {
  ...
  let idTP = $("#seccion_id").val();
  let nom = $("#nombreSecciones").val(); 
  let des = $("#des").val();
  let select = $("#selectmod").val();
  let icon = $("#iconSecciones").val();
  let url = $("#urlSecciones").val();
  let orden = $("#ordenSecciones").val();
  ...
  if ("" == nom.trim()) {
    $("#errornombreSecciones").show();
    $("#errornombreSecciones").html("El campo no debe ir vacio");
    $("#nombreSecciones").focus();
    goValidation = false;
    habilitaCampos();
  }
  ...
  if ("" == des.trim()) {
    $("#errordes").show();
    $("#errordes").html("El campo no debe ir vacio");
    $("#des").focus();
    goValidation = false;
    habilitaCampos();
  }
  ...
  if(select.length == 0 || select[0] == ""){
    $('#errorSelect').show();
    $('#errorSelect').html("Selecciona un módulo");
    $('#selectmod').focus();	
    goValidation = false;
    habilitaCampos();
  }  
  ...
  if ("" == icon.trim()) {
    $("#errorIcono").show();
    $("#errorIcono").html("El campo no debe ir vacio");
    $("#iconSecciones").focus();
    goValidation = false;
    habilitaCampos();
  }
  ...
  if ("" == url.trim()) {
    $("#errorURL").show();
    $("#errorURL").html("El campo no debe ir vacio");
    $("#urlSecciones").focus();
    goValidation = false;
    habilitaCampos();
  }
  ...
  if ("" == orden.trim()) {
    $("#errorOrden").show();
    $("#errorOrden").html("El campo no debe ir vacio");
    $("#ordenSecciones").focus();
    goValidation = false;
    habilitaCampos();
  } else if (!orden.match(ExpRegSoloNumeros)) {
    $("#errorOrden").show();
    $("#errorOrden").html("El campo debe ser numérico");
    $("#ordenSecciones").focus();
    goValidation = false;
    habilitaCampos();
  }
  ...
  if (goValidation) {
    axios.post(base_url() + "daniw/Secciones/insertaSecciones", {
        seccion_id  : idTP,
        nombre_sec  : nom,
        des_seccion : des,
        ico_sec     : icon,
...
function agregar() {
  ...
  $("#nombreModal").html("Agregar nueva sección");
  $("#nombreSecciones").val("");
  $("#des").val("");
  $("#iconSecciones").val("");
  $("#urlSecciones").val("");
...
function editar(id, nom, des, icon, url, orden, mod) {
  quitaErroresCamposVacios();
  listaModulos(mod);
  ...
  $("#accion").val("editar");
  $("#ModalAgregar").modal("show");
  $("#nombreModal").html("Editar Sección");
  $("#nombreSecciones").val(nom);
  $("#des").val(des);
  $("#iconSecciones").val(icon);
  $("#urlSecciones").val(url);
  $("#ordenSecciones").val(orden);
...
} // temrina modal editar secciones

function cambiaEstatus(id) {
  $.ajax({
    url: base_url() + "daniw/Secciones/cambioEstatus",
    dataType: "JSON",
    type: "POST",
    data: {
      id: id,
...
function listaModulos(mod){
  $.ajax({
    "url":base_url()+"daniw/Secciones/getModulos",
    "dataType":"JSON"
  })
  .done((data)=>{ 
    ...
    if(data.resultado){
        ...
        $("#divSelectSecciones").find("select").append(`<option value="Selecciona">--Selecciona--</option>`);

        $.each(data.Modulos, function(i,o){

            if(o.estatus == 1){
                $("#divSelectSecciones").find("select").append(
                `<option value="` + o.modulo_id + `"` + (o.nombre_mod === mod ? "selected" : "") +`>` + o.nombre_mod +`</option>`
                );
            }  
            
        });
    }else{
      $("#divSelectCategoriasServicios").find("select").append(`<option value="Selecciona">--No existen módulos para mostrar--</option>`);
    }
  })
  .fail();
}

// function selectModulo(id){
...
//  $.ajax({
//      "url":base_url()+"daniw/Secciones/getSelect",
//      "dataType":"JSON",
//      "type":"POST",
//      "data":{
//          id:id
...
//       .done((data)=>{
//         if(data.resultado){
//           $("#divSelectSecciones").find("select").html(`
//           <option label="&nbsp;">&nbsp;</option>`);
//           $.each(data.Modulos, function(i,o){
...
//                 if(o.modulo_id == n.modulo_id){
//                   $("#divSelectSecciones").find("select").append(`
//                   <option value="`+ o.modulo_id+`" Selected >`+ o.nombre_mod+`</option>`);
//                   bandera = 1;
//                 }   
...
//             $("#divSelectSecciones").find("select").append(`
//             <option value="Selecciona">--No existen módulos para mostrar--</option>
//             `
//             );
//         }
//       })
...
function btnModalBorrar() {
  ...
  $.ajax({
    url: base_url() + "daniw/Secciones/bajaLogica",
    dataType: "JSON",
    type: "POST",
    data: {
      id: id,
