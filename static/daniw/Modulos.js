$(document).ready(() => {
  ...
  listaModulos();
  //console.log("Prueba: "+listaModulos);
});

function listaModulos() {
  axios(base_url() + "daniw/Modulos/verModulos")
    .then(({ data }) => {
      if (data.resultado) {
        ...
        $.each(data.Secciones, function (i, o) {
          $("#datatable").find("tbody").append(
              `<tr id="tr-`+ o.seccion_id +`">
                ...
                <td>` + o.nombre_mod  + `</td>
                <td>` + o.des_modulo + `</td>
						    <td> <i class="` + o.ico_mod + `"></i></td>
                <td>` + o.orden     + `</td>
                <td align="center">
                  <a href="#" onclick="cambiaEstatus(` + o.modulo_id + `)">
                    <i id="icono-` + o.modulo_id + `" class="fas fa-toggle-` + (o.estatus == 1 ? "on" : "off") + ` fa-2x"></i>
                  </a>
                </td>
                <td align="center">
                  <a href="#" onclick="editar(` + o.modulo_id + `,'` + o.nombre_mod + `','` + o.des_modulo + `','` + o.ico_mod + `','` + o.orden + `')">
                    <i class="fas fa-pencil fa-2x"></i>
                  </a> 
                </td>
                <td align="center">
                  <a href="#" onclick="modalBorrar(` + o.modulo_id + `,'` + o.nombre_mod + `','` + o.des_modulo + `')">
...
function desabilitaCampos(){
  $("#btnEnviar, #nombreModulos, #des, #iconModulos, #ordenModulos").attr("disabled", "disabled");
}

// Input and select 
function habilitaCampos(){
  $("#btnEnviar, #nombreModulos, #des, #iconModulos, #ordenModulos").removeAttr("disabled"); 
}

function insertaModulos() {
  ...
  let idTP = $("#modulo_id").val();
  let nom = $("#nombreModulos").val();
  let des = $("#des").val();
  let icon = $("#iconModulos").val();
  let orden = $("#ordenModulos").val();
  ...
  if ("" == nom.trim()) {
    $("#errornombre").show();
    $("#errornombre").html("El campo no debe ir vacio");
    $("#nombreModulos").focus();
    goValidation = false;
    habilitaCampos();
  }
  ...
  if (goValidation) {
    axios.post(base_url() + "daniw/Modulos/insertaModulos", {
        modulo_id   : idTP,
        nombre_mod  : nom,
        des_modulo  : des,
        ico_mod     : icon,
...
function agregar() {
  ...
  $("#nombreModal").html("Agregar nuevo módulo");
  $("#nombreModulos").val("");
  $("#des").val("");
  $("#iconModulos").val("");
  $("#ordenSecciones").val("");
...
function editar(id, nom, des, icon, orden, estatus) {
  quitaErroresCamposVacios();

  $("#btnEnviar").html("Actualizar");
  $("#accion").val("editar");
  $("#ModalAgregar").modal("show");
  $("#nombreModal").html("Editar módulo");
  $("#nombreModulos").val(nom);
  $("#des").val(des);
  $("#iconModulos").val(icon);
  $("#ordenModulos").val(orden);
...
function cambiaEstatus(id) {
  $.ajax({
    url: base_url() + "daniw/Modulos/cambioEstatus",
    dataType: "JSON",
    type: "POST",
    data: {
      id: id,
...
function btnModalBorrar() {
  ...
  $.ajax({
    url: base_url() + "daniw/Modulos/bajaLogica",
    dataType: "JSON",
    type: "POST",
    data: {
      id: id,
