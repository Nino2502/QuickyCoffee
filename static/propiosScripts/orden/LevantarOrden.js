$(document).ready(() => {
    ...
    //alert("Soy Inventario Quicky Coffee");
    ...
    $("#btnAgregarServicio").click((e) => {
        ...
        if (validaSeleccionServicio == true) {
            ...
            $.ajax({
                "url": base_url() + "app/orden/LevantarOrden/verServicio",
                "dataType": "JSON",
                "type": "POST",
                "data": {
                    "id": idServicio
                ...
                .done((data) => {
                    ...
                    if (validaCarrito(idServicio)) {
                        ...
                        console.log("Entre a validar carrito", idServicio);

                        if (data.Servicio[0].idForm != null && data.Servicio[0].idForm != 0) {
							console.log("Tengo idform 0");
                            console.log("Entre a su formulario");

                            let idForm = data.Servicio[0].idForm
                            idFormulario = idForm

                            console.log("LLevo a idForm", idFormulario);


                            $("#modalDiagnostico").modal("show");
                            $.each(data.formulario, function (i, o) {
                                $("#formularioQuestion").append(`
                          
                                <label for="message-text" class="col-form-label">`+ o.nombrePD + `:</label>
                                <textarea class="form-control" rows="4" id="`+ o.idPD + `"></textarea>
                            `
                                )
                            });
                        ...
                        if (data.resultado) {
                            ...
                            precioBase = parseFloat(data.Servicio[0].precioS);
                            ...
                            registrarCarrito(idServicio);


                            console.log("Soy el id que va a entrar a al if", idServicio);

                            let idservicios = data.Servicio[0].idS;

                            let idtipoSP = data.Servicio[0].tipoSP;


                            console.log("Te dire si soy producto o servicio", idtipoSP);



                            console.log("Soy id sercion declarado", idservicios);
                            ...
                            console.log("Soy id sercion declarado", idservicios);
                                ...
                                console.log("entonces soy servicio");
                                ...
                                //$("#tecnicoServicio").hide();

                                $("#bodySeleccionados").append(`
                                    <tr propser  class="filaSelecionada" id="tr-`+ idServicio + `">
						     
							 
							

						
                            <td style="text-align: center" data  scope="row" id="idSelecionado" colspan="1">`+ data.Servicio[0].idS + `</td>
                            ...
                            <td data id="precio-${idServicio}" style="text-align: center" colspan="3">$` + precioBase + `</td>
    ...
    $("#btnVentaRapida").click((e) => {
        ...
        if (validaSeleccionServicio == true) {
            ...
            $.ajax({
                "url": base_url() + "app/quicky/LevantarOrden/verServicio",
                "dataType": "JSON",
                "type": "POST",
                "data": {
...
function removerFormulario(idServicio) {
    preguntasFormulario = preguntasFormulario.filter((pregunta) => {
        return pregunta.idServicio !== idServicio; // retorna todo lo que no sea igual a la ID
    });
}
...
function respuestaFormulario() {
    ...
    let idServicio = $("#selectListaServicios").val();
    // Obtener el valor de "Descripción"
    let textoServicio = $("#selectListaServicios option:selected").text();
    let descripcionStartIndex = textoServicio.indexOf("Descripción: ");
    let descripcionEndIndex = textoServicio.indexOf(" Stock:");
    let descripcion = textoServicio.substring(descripcionStartIndex + 12, descripcionEndIndex);
    ...
    $('#formularioQuestion textarea').each(function () {
        var pregunta = $(this).prev('label').text();
        var respuesta = $(this).val();
        formulario[pregunta] = respuesta;
    });
    formulario["idServicio"] = idServicio;
    formulario["servicio"] = descripcion;
    formulario["idFormulario"] = idFormulario;

    console.log("soy idFormulario", idFormulario);


    let formularioExistente = preguntasFormulario.find(f => f.servicio === idServicio);

    if (formularioExistente) {
        toastr["error"]("Formulario ya agregado");
        $("#modalDiagnostico").modal("hide");
    } else {
        preguntasFormulario.push(formulario);
        $("#modalDiagnostico").modal("hide");
        toastr["success"]("Formulario agregado");
    }

    console.log("preguntasFormulario", preguntasFormulario);
}
...
function listaServicios() {
    ...
    axios(base_url() + "app/orden/LevantarOrden/verServicios")
        .then(({ data }) => {
                ...
                $.each(data.Servicios, function (i, o) {
                    ...
                    if (o.estatus == 1) {
                        ...
                        $("#divListaServicios").find("select").append(`
                            <option value=` + o.idS + `> ` + "Productos : " + servicio + " . Descripcion : " + o.desS + ` </option>
                        `);
                    }
                });
...
function tiposPagos() {
    ...
    $.ajax({
        url: base_url() + "app/orden/LevantarOrden/TiposPagos",
        dataType: "JSON",
        type: "GET",
    })
        .done((data) => {
...
function infoCliente() {
    ...
    $.ajax({
        url: base_url() + "app/orden/LevantarOrden/getUser",
        dataType: "JSON",
        type: "POST",
        data: {
            idU: User
...
const insertaAgrupaServicios = async () => {
    ...
    console.log("Le di click a agregar carrito");
    ...
    $("#modalCarrito").modal({
        backdrop: true, // Habilitar el backdrop
        keyboard: false // Deshabilitar cierre con Esc
    });
    ...
    const convertedArrays = CarritoProductos.map(function (subarreglo) {
        if (subarreglo.length === 10) {
            console.log("Entre en subArreglo de 10");
            return {
                idServicio: subarreglo[0],
                Stock: subarreglo[1],
                PrecioBase: subarreglo[2].replace('$', ''),
                CantidadMedioMayoreo: subarreglo[3],
                PrecioMedioMayoreo: subarreglo[4].replace('$', ''),
                CantidadMayoreo: subarreglo[5],
                PrecioMayoreo: subarreglo[6].replace('$', ''),
                Tipo: subarreglo[7],
                Cantidad: subarreglo[8],
                subtotal: subarreglo[9]
            };
        } else {
            console.log("Entro en subarreglo 10 que es 2");

            console.log("Soy el convertedArrays   ", subarreglo);
            return {
                idServicio: subarreglo[0],
                PrecioBase: subarreglo[1].replace('$', ''),
                Cantidad: subarreglo[2],
                subtotal: subarreglo[3]
            };
    ...
    convertedArrays.forEach(item => {
        ...
        console.log("Soy el result de carrito      ", result );
    });
    ...
    $("#btnRealizarVenta").off("click").on("click", (e) => {
        ...
        console.log("Soy boton de realizar venta modal");
        ...
        */
		...
		if(goValidation == true){
			...
			console.log("Entre en  goValidation == true");
			
			
			$.ajax({
					url: base_url() + "app/orden/LevantarOrden/OrdenCompra",
					dataType: "JSON",
					type: "POST",
					data: formbody,
				})
				...
				console.log("Ocurrio un error mayor");
				console.log("Imposible generar la compra");
...
/*
const costo_atencion_servicio = async () => {
    console.log("entro en costo_atencion_servicio")
    formbody = {
        id_atencion_servicio: 34
    }
    await $.ajax({
        url: base_url() + "app/quicky/LevantarOrden/obtener_costo_atencion",
        dataType: "JSON",
        type: "POST",
        data: formbody,
    })
        .done((data) => {
            console.log("soy data de atencion", data)
            console.log("costo atencionnnn",  data.costo_atencion_servicio.costo)
            costo_atencion =   data.costo_atencion_servicio.costo
...
function verificaUsuario() {
    ...
    $.ajax({
        url: base_url() + "app/orden/LevantarOrden/VerificaUsuario",
        dataType: "JSON",
        type: "GET",
...
function registrarUsuario() {
    ...
    if (goValidation) {
        ...
        $.ajax({
            url: base_url() + "app/orden/LevantarOrden/registrarNuevoCliente",
            dataType: "JSON",
            type: "POST",
            data: formbody,
        })
...
function consultarSuc(idServ, nombre) {
    ...
    $("#titleStock").html("Formulario diagnostico" + "<br>" +"<strong>" +  nombre + "</strong>");
    $("#SucursalesInventario").append(`

						
						<label for="message-text" class="col-form-label col-12">¿Que marca es tu equipo?</label>
						<br>
    					<label for="message-text" class="col-form-label col-12">¿Que capacidad tiene tu dispositivo?</label>
						<br>
						<label for="message-text" class="col-form-label col-12">¿Que sistema operativo tiene?</label>
						<br>
						<label for="message-text" class="col-form-label col-12">¿Se a mojado tu equipo con anterioridad?</label>
                    `);
				   

    $.ajax({
        url: base_url() + "app/orden/LevantarOrden/consultarProducto",
        dataType: "JSON",
        type: "POST",
        data: {
            idS: idServ
...
function AgregarCambio() {
    ...
    $("#btnverificarAcceso").off("click").on("click", (e) => {
        ...
        $.ajax({
            url: base_url() + "app/orden/LevantarOrden/Acceder",
            dataType: "JSON",
            type: "POST",
            data: formbody,
        })
    ...
    $.ajax({
        url: base_url() + "app/orden/LevantarOrden/getTM",
        dataType: "JSON",
        type: "GET"
    })
        .done((data) => {
...
function añadirCambio() {
    ...
    if (bandera) {
        ...
        $.ajax({
            url: base_url() + "app/orden/LevantarOrden/CambioCaja",
            dataType: "JSON",
            type: "POST",
            data: {
...
const obtenerPuntos = async (cliente) => {
    ...
    await $.ajax({
        url: base_url() + "app/orden/ProgramaLealtad/obtenerPuntos",
        dataType: "JSON",
        type: "POST",
        data: {
            "cliente": cliente
...
//     await $.ajax({
//         url: base_url() + "app/quicky/ProgramaLealtad/programaActual",
//         dataType: "JSON",
//         type: "GET"
//         })
//         .done((data)=>{
...
const consultarPrograma = () => {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: base_url() + "app/orden/ProgramaLealtad/programaActual",
            dataType: "JSON",
            type: "GET"
        })
            .done((data) => {
...
function registrarCarrito(idServicio) { // push data array para generar el id general de cada productos registrados.

    let existeProducto = datos.some(r => r.Servicio === idServicio);
    let servicio = 0;

    if (!existeProducto) {
        console.log("Entre 1 registra carrito", existeProducto);
        ...
        console.log("entre en 2 en validar carrito");
...
function tecnicos_Servicio() {
    let tecnico = $("#selectTecnico");
    console.log("Soy Tecnico id", tecnico);

    $.ajax({
        url: base_url() + "app/orden/LevantarOrden/tipos_tecnicos",
        dataType: "JSON",
        ...
        .done((data) => {
            ...
            `);
                ...
                $.each(data.Tecnicos, function (i, o) {
                    if (o.estatus == 1) {
                        $("#selectTecnico").append(`
					<option value="` + o.idU + `"` + (o.idU == tecnico ? " selected" : "") + `>` + o.nombreU + ' ' + o.correo + `</option>
				`);
                ...
                <option value="Selecciona">No Hay Tecnicos</option>
