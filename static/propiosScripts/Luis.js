...
function listaBecario(){
    ...
    axios(base_url()+"app/Luis/verBLuis")
    .then(({data})=>{
        ...
        if(data.resultado){
            ...
            $.each(data.Becarios, function(i,o){
...
function insertaBLuis(){
    ...
    if(goValidation){
        ...
        axios.post(base_url()+"app/Luis/insertaBLuis", {
            idBe:idBe,
            nombre: nom,
            descripcion: des,
            estatus: estatus,
...
function cambiaEstatus(id){
    ...
    $.ajax({
        "url": base_url()+"app/Luis/cambioEstatus",
        "dataType": "JSON",
        "type":"POST",
        "data":{
            "id":id
