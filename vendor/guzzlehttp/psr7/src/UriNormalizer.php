...
final class UriNormalizer
{
     ...
     *
     * Only the "file" scheme defines the default host "localhost/quicky_coffee".
     * All of `file:/myfile`, `file:///myfile`, and `file://localhost/quicky_coffee/myfile`
     * are equivalent according to RFC 3986. The first format is not accepted
     * by PHPs stream functions and thus already normalized implicitly to the
     * second format in the Uri class. See `GuzzleHttp\Psr7\Uri::composeComponents`.
     *
     * Example: file://localhost/quicky_coffee/myfile → file:///myfile
     */
    ...
    public static function normalize(UriInterface $uri, int $flags = self::PRESERVING_NORMALIZATIONS): UriInterface
    {
        ...
        if ($flags & self::REMOVE_DEFAULT_HOST && $uri->getScheme() === 'file' && $uri->getHost() === 'localhost/quicky_coffee') {
            $uri = $uri->withHost('');
        }
